...
export const BusinessUnitProvider: React.FC<BusinessUnitProviderProps> = ({ children }) => {
  const { customerDetails, isLoggedIn, setAuthState } = useAuthContext();
  ...
  const [businessUnitStoresCache, setBusinessUnitStoresCache] = useState<Record<string, Store[]>>({});
  ...
  const { refetch: getBusinessUnitStores } = useMcQuery<GetBusinessUnitStoresResponse>(
    GET_BUSINESS_UNIT_STORES,
    {
      variables: {
        id: '',
      },
  ...
  const fetchStoreInfo = async (businessUnitId: string): Promise<Store[] | undefined> => {
    try {
      ...
      console.error(`Error fetching stores for business unit ${businessUnitId}`);
      return undefined;
    }
  };

  // Update auth context with store information more efficiently
  const updateStoreContext = useCallback(async (businessUnit: BusinessUnit) => {
    ...
    try {
      ...
      let stores = businessUnitStoresCache[businessUnit.id] || await fetchStoreInfo(businessUnit.id);
      ...
      if (stores && stores.length > 0) {
        ...
        console.log(`Active store: "${storeName}" (${storeKey}) for business unit "${businessUnit.name}"`);
        
        // Update auth context with the store key
        if (isLoggedIn && customerDetails) {
          // Only update if the store key has changed
          if (customerDetails.storeKey !== storeKey) {
            setAuthState(
              isLoggedIn,
              {
                ...customerDetails,
        ...
        console.warn(`No stores found for business unit ${businessUnit.name}`);
      }
    } catch (error) {
      console.error('Error updating store context');
    }
  }, [isLoggedIn, customerDetails, setAuthState, businessUnitStoresCache, fetchStoreInfo]);
  ...
  const handleSelectBusinessUnit = useCallback((businessUnitId: string) => {
